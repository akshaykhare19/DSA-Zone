...
the function polyMultiply() which takes the array Arr1[] and Arr2[]and 
their size M and N as input parameters and returns the polynomial in form 
of array formed after multiplication of given polynomials..
    ...
    public int[] polyMultiply(int Arr1[], int Arr2[], int M, int N)
    {
        ...
        for(int k = 0; k < M+N-1; k++){
            for(int i = 0; i < M; i++){
                for(int j = 0; j < N; j++){
                    if(i+j == k){
                        result[k] = result[k] + (Arr1[i] * Arr2[j]);
                    }
                }
            }
        }
